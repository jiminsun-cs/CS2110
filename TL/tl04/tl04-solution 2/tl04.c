...
int set_character(struct ascii_image *image, int row, int col, char c) {
    ...
    if (row < 0 || row >= image->height || col < 0 || col >= image->width) return FAILURE;
 ...
 * For this reason, it is HIGHLY RECOMMENDED to use set_character, which refuses to draw anything if the provided coordinates is out-of-bounds.
...
int draw_hollow_box(struct ascii_image *image, int row, int col, int height, int width, char c) {
    if (image == NULL) return FAILURE;
    if (height < 1 || width < 1) return FAILURE;
    for (int r = 0; r < height; r++) {
        set_character(image, row + r, col, c);
        set_character(image, row + r, col + width - 1, c);
    }
    for (int cc = 0; cc < width; cc++) {
        set_character(image, row, col + cc, c);
        set_character(image, row + height - 1, col + cc, c);
    }
    return SUCCESS;
}
...
struct ascii_image *create_image(int height, int width, char *name) {
    if (height < 1 || width < 1 || name == NULL) return NULL;
    ...
    image->name = copy;
    image->data = data;
    for (int i = 0; i < height*width; i++) image->data[i] = '.';
    return image;
}
...
int add_extension(struct ascii_image *image, char *extension) {
    // Going to deliberately avoid strcat here since we didn't teach it
    if (image == NULL || extension == NULL) return FAILURE;
    char *concat = malloc(strlen(image->name) + strlen(extension) + 1);
    if (!concat) return FAILURE;
    strcpy(concat, image->name);
    strcpy(concat + strlen(image->name), extension);
    free(image->name);
    image->name = concat;
    return SUCCESS;
}
