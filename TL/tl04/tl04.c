...
int draw_hollow_box(struct ascii_image *image, int row, int col, int height, int width, char c) {
    ...
    for (int i  = 0; i < height; i++) {
        if ((row + i == row) || (row + height - 1 == row + i)) { // if at very top and very bottom
            for (int j = 0; j < width; j++) {
                set_character(image, (row+i), (col + j), c);
            }
        } else {
            set_character(image, (row + i), (col), c);
            set_character(image, (row + i), (col + width - 1), c);
        }
    }
    return SUCCESS;
}
...
// dynamically allocate space for new image, new image's name, new images'data
    if (width < 1) {
        return NULL;
    }
    if (height < 1) {
    ...
    struct ascii_image *image = (struct ascii_image*) malloc(sizeof(struct ascii_image));
    if (image == NULL) {
        return NULL;
    }
    //name
    ...
    // image->data = (char*) malloc(sizeof(image->data));
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
           //int set_character(struct ascii_image *image, int row, int col, char c) {
           set_character(image, (height + i), (width + j), '.');
        }
    }
    return image;
...
void destroy_image(struct ascii_image *image) {
    // TODO: IMPLEMENT THIS FUNCTION! SEE .pdf FOR DETAILS
    //deallocate ascci image struct, deep copied name, image data array
    if (image == NULL) {
        return;
    }
    if (image->data == NULL) {
...
int add_extension(struct ascii_image *image, char *extension) {
    ...
    //extending to image name, dynamically allocate new pointer
    //point should be assigned to image->name
    if (image == NULL) {
        return FAILURE;
    }
    if (extension == NULL) {
    ...
    char *replacement = realloc(image->name, strlen(image->name) + strlen(extension) + 3);
    if (replacement == NULL) {
        return FAILURE;
    }
    strcat(image->name, extension);
    strcpy(replacement, image->name);
    return SUCCESS;
}
